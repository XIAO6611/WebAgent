...
document.addEventListener("DOMContentLoaded", () => {
  chrome.storage.local.get(["apiKey", "knowledgeBase", "resumeText"], (result) => {
    ...
    if (result.developerMode) document.getElementById("developerMode").checked = true; // 读取状态
...
document.getElementById("saveBtn").addEventListener("click", () => {
  const apiKey = document.getElementById("apiKey").value.trim();
  const resumeText = document.getElementById("resumeText").value.trim();
  const developerMode = document.getElementById("developerMode").checked; // 获取开关状态
