// 统一接收 config: { apiKey, apiBaseUrl, vlmModel, llmModel }
export async function askVLM(config, systemPrompt, screenshotBase64, externalSignal) {
...
export async function askLLM(config, summaryPrompt, externalSignal) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 120000);
  const signal = externalSignal || controller.signal;
  const keepAlive = setInterval(() => chrome.storage.local.get('keepAlive'), 20000);
...
export async function askTextJson(config, prompt, signal) {
  const content = await askLLM(config, prompt, signal);
  return parseModelJson(content);
}

// parseModelJson, extractJsonObjectText, repairLooseJson 保持不变
